...
public class DiskSyncTask extends AsyncTask<Void, String, String> {
	...
	private static final String CURRENT_PERMISSIONS = "current_permission";
    	...
    	try {
	        ...
	        if (formDir.exists() && formDir.isDirectory()) {
	            ...
	            //         discard files not ending with ".xml" or ".xhtml"
	            {
	            	...
	            	for ( File addMe: formDefs ) {
	                    ...
	                    if (!addMe.getName().startsWith(".")
							...
							if(permissionIsAvailable(addMe)) {
		            ...
		            while (mCursor.moveToNext()) {
		                ...
		                if (sqlFile.exists()) {
		                    ...
		                    if (!FileUtils.getMd5Hash(sqlFile).contentEquals(md5)) {
								...
								if(permissionIsAvailable(sqlFile)) {
									uriToUpdate.add(new UriFile(updateUri, sqlFile));
								}
		                    }
		                } else {
		                	Log.d(t, "["+instance+"] file referenced by content provider does not exist " + sqlFile);
		                }
		            }
		        } finally {
		        	if ( mCursor != null ) {
		        ...
		        for ( UriFile entry : uriToUpdate ) {
					...
					if(	permissionIsAvailable(formDefFile)) {
	...
	private boolean permissionIsAvailable(File file) {
		...
		for (String permission: permissionList){
			if(file.getPath().contains(permission)){
				return true;
			}
		}
		return false;
